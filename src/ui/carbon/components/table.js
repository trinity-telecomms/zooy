...
const initEventHandlers = (panel, el, attrs) => {
  const dataBinder = el.dataBinder;
  ...
  if (dataBinder) {
    el.addEventListener('cds-table-header-cell-sort', e => {
      ...
      dataBinder.fetchData(params).catch(err => {
  ...
  if (searchElement) {
    if (dataBinder) {
      ...
      panel.listen(searchElement, 'keydown', e => {
        if (e.key === 'Enter') {
          skeleton && skeleton.classList.remove('hidden');
          pagination.totalItems = 0;
          el.querySelector('cds-table-body').replaceChildren();
...
const createPagination = (el, dataBinder) => {
   ...
   *
   * @param {DataBinder} dataBinder - DRF API response with count, next, previous, results
   */
  pagination.setData = (dataBinder) => {
