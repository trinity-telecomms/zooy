...
1. Carbon's `cds-contained-list-item` fires `cds-contained-list-item-click` (bubbling, composed) — but _only_ when the item has the `clickable` attribute set.
...
3. **Nested clickables don't collide (usually).** Carbon's `action` slot on items renders _outside_ the clickable shadow button, so a trailing `cds-icon-button slot="action"` can have its own `event` without triggering the item's click. But anything inside the default slot _is_ inside the button and will trigger both.
